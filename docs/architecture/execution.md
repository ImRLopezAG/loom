...
U7 still needs locked database execution/history, drift checks, transactional recovery, role separation, custom/nontransactional SQL handling and versioned metadata bootstrap. U8-U20 remain unimplemented; no full runtime, migration lifecycle, browser interaction, cloud, capacity, or release acceptance is claimed by these checks.
