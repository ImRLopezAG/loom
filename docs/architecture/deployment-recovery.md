...
Framework metadata version 20 adds a `retired` activation state. Normal grant preparation and inspection refuse it; activation uses an atomic state predicate that can only activate quarantined or already-active grants. Runtime startup also refuses retired grants. Release planning reports `RUNTIME_RETIRED` for the matching deployment/source version, including when a new release key is used. Retirement state belongs to the grant and is separate from a retired ingress claim, which deliberately leaves an older worker available for queued jobs. No public retirement command sets this state yet: coordinated dependency checks, drainage and provider cleanup are still required. An owner manually changing the state is not proof of safe retirement.
