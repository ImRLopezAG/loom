...
test.skipIf(!connectionString)("release planning retains its committed baseline after live schema evolution and preserves renamed data", async () => {
  ...
  const expanded = defineSchema((f) => ({ tasks: { title: f.text().notNull(), description: f.text() } }), { namespace });
  try {
    ...
    const renamed = defineSchema((f) => ({ tasks: { name: f.text().notNull(), description: f.text() } }), { namespace });
    ...
    expect((await pool.query<{ name: string }>(`SELECT name FROM "${namespace}".tasks`)).rows).toEqual([{ name: "preserve me" }]);
...
test("programmatic Drizzle rename resolution does not write progress to stdout", async () => {
  ...
  const child = Bun.spawn([process.execPath, "--eval", source], { cwd: fileURLToPath(new URL("..", import.meta.url)), stdout: "pipe", stderr: "pipe" });
