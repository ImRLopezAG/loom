...
test.skipIf(!connectionString)("native RQB v2 covers self, multiple-target and junction relations inside transactions", async () => {
  ...
  const schema = defineSchema((s) => ({
    ...
    tasks: { title: s.text().notNull(), projectId: s.reference("projects").notNull(), authorId: s.reference("users").notNull(), reviewerId: s.reference("users") },
    memberships: defineTable({ userId: s.reference("users").notNull(), projectId: s.reference("projects").notNull() }, { indexes: [{ fields: ["userId", "projectId"], unique: true }] }),
  }), { namespace });
  const relations = defineRelations(schema.tables, (r) => ({
    users: {
      manager: r.one.users({ from: r.users.managerId, to: r.users._id }),
      projects: r.many.projects({ from: r.users._id.through(r.memberships.userId), to: r.projects._id.through(r.memberships.projectId) }),
    },
    tasks: {
  ...
  try {
    ...
    try {
      ...
      const [author] = await db.insert(schema.tables.users).values({ name: "Author", managerId: manager._id }).returning();
      ...
      const rows = await db.transaction((tx) => tx.query.tasks.findMany({
        where: { project: { name: "Loom" } }, orderBy: { title: "asc" },
        columns: { title: true }, with: { project: { columns: { name: true } }, author: { with: { manager: true, projects: true } }, reviewer: true },
