...
test("configuration has bounded defaults and rejects unknown settings and protected namespaces", () => {
  ...
  expect(() => defineConfig({ project: "tasks", database: { runtimeUrlEnv: "DATABASE_URL", migrationUrlEnv: "DATABASE_URL" } })).toThrow("separate");
...
test("project paths reject traversal and symlink escapes without requiring the output to exist", async () => {
  ...
  try {
    ...
    expect(await resolveProjectPath(root, "backend/new/file.ts")).toBe(join(await realpath(root), "backend/new/file.ts"));
