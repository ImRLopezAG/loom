...
test("migration planning is deterministic and separates safe additions from required backfills", async () => {
  ...
  const required = await planMigration(before, defineSchema((f) => ({ tasks: { title: f.text(), description: f.text().notNull() } })));
...
test("migration planning refuses rename guesses and marks explicit renames for review", async () => {
  ...
  const plan = await planMigration(before, renamed, [{ type: "rename", kind: "column", from: ["public", "tasks", "title"], to: ["public", "tasks", "name"] }]);
...
test("migration planning treats constraint, type and deletion changes as reviewed changes", async () => {
  ...
  const changed = await planMigration(before, defineSchema((f) => ({ tasks: { title: f.text().unique(), count: f.bigint() } })));
  ...
  const dropped = await planMigration(before, defineSchema((f) => ({ tasks: { title: f.text() } })));
...
test("committed migration artifacts reject altered SQL and broken lineage", async () => {
  ...
  try {
    const schema = defineSchema((f) => ({
      projects: { name: f.text().unique() },
      tasks: defineTable({ title: f.text(), projectId: f.reference("projects"), state: f.enum(["open", "closed"]),
        amount: f.numeric({ precision: 12, scale: 2 }), metadata: f.json() }, { indexes: [{ fields: ["title", "state"] }] }),
    }), { namespace: "app" });
    ...
    const sqlFile = join(root, "migrations", `${initial.hash}_initial`, "migration.sql");
    const sql = await readFile(sqlFile, "utf8");
    await writeFile(sqlFile, sql + "\nDROP SCHEMA app CASCADE;\n");
    await expect(readMigrations(root, "migrations")).rejects.toThrow("SQL artifact");
    await writeFile(sqlFile, sql);
    const planFile = join(root, "migrations", `${initial.hash}_initial`, "plan.json");
