...
const relations = defineRelations(schema.tables, (r) => ({ tasks: {
...
const result = await connection.db.transaction((tx) => tx.query.tasks.findFirst({
  with: { author: true, reviewer: true }, where: { author: { name: "Angel" } },
}));
if (result) {
