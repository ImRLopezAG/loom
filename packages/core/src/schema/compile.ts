...
import {
  check, customType, foreignKey, index, pgSchema, pgTable, uniqueIndex, uuid,
...
function systemColumns<Name extends string>() {
  return {
    _id: uuid("_id").$type<Id<Name>>().primaryKey().default(sql`uuidv7()`),
    _createdAt: milliseconds("_createdAt").notNull().default(sql`floor(extract(epoch from clock_timestamp()) * 1000)`),
  };
}
...
export interface CompiledField extends FieldMetadata { readonly name: string; readonly sqlName: string }
...
export interface SchemaMetadata { readonly namespace: string; readonly entities: readonly CompiledEntity[] }
export interface CompileOptions { readonly namespace?: string }
interface ColumnBuilders extends Record<string, AnyPgColumnBuilder> {}

export function compile<const Entities extends Record<string, EntityDeclaration>>(entities: Entities, options: CompileOptions) {
  ...
  const metadata: SchemaMetadata = Object.freeze({ namespace, entities: Object.freeze([...declarations].map(([name, table]) => {
    ...
    const compiledFields = Object.entries(table.fields).sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0).map(([key, field]) => {
      const path = `${name}.${key}`;
      ...
      if (reference && !declarations.has(reference.target)) throw new Error(`Unknown reference at ${path}: ${reference.target}`);
      if (reference?.onDelete === "set null" && notNull) throw new Error(`Cannot set null on required reference: ${path}`);
      return Object.freeze({ name: key, sqlName: columnName, kind, notNull, unique,
  ...
  for (const entity of metadata.entities) {
    ...
    tables[entity.name] = createTable(entity.sqlName, columns, (built) => {
      ...
      for (const field of entity.fields) {
        ...
        if (field.enumValues) {
          constraints.push(check(constraintName(entity.sqlName, field.sqlName, "enum"), sql`${column} in (${sql.join(field.enumValues.map((value) => sql`${value}`), sql`, `)})`.inlineParams()));
        }
      }
  ...
  return Object.freeze({ tables: Object.freeze(nativeTables), metadata,
...
function constraintName(table: string, field: string, suffix: string): string {
  ...
  return name.length <= 63 ? name : `${name.slice(0, 46)}_${createHash("sha256").update(name).digest("hex").slice(0, 16)}`;
}
