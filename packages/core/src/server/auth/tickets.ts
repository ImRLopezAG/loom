...
export function createConnectionTickets(options: ConnectionTicketOptions) {
  const { db, deployment, metadataNamespace } = options;
  ...
  return {
    async issue(session: VerifiedSession, origin: string): Promise<ConnectionTicket> {
      ...
      const result = await db.execute<{ expiresAt: number }>(sql`
        INSERT INTO ${table} (deployment, ticket_hash, origin, identity, session_expires_at, expires_at)
        SELECT ${deployment}, ${digest(ticket)}, ${origin}, ${JSON.stringify(captured.identity)}::jsonb,
    ...
    async redeem(ticket: string, origin: string): Promise<VerifiedSession> {
      ...
      const result = await db.execute(sql`
        WITH consumed AS (
          ...
          RETURNING identity, expires_at, session_expires_at
        )
        SELECT identity, extract(epoch FROM session_expires_at)::float8 AS "expiresAt" FROM consumed
