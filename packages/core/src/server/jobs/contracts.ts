...
export const scheduleOptions = v.strictObject({
  ...
  dueAt: v.date(),
  maxAttempts: v.optional(v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(10)), 1),
  retryDelaySeconds: v.optional(v.pipe(v.number(), v.integer(), v.minValue(0), v.maxValue(3600)), 1),
});
...
export const leaseDuration = v.pipe(v.number(), v.integer(), v.minValue(1), v.maxValue(300));
