...
import { TableDefinition } from "../schema/table.js";
import type { EntityDeclaration } from "../schema/table.js";
import { storageParser, systemParsers } from "./encoding.js";
import type { StorageRow } from "./encoding.js";
import type { SchemaValidators } from "./types.js";
...
export function derive<const Entities extends Record<string, EntityDeclaration>>(entities: Entities, metadata: SchemaMetadata) {
  ...
  for (const entity of metadata.entities) {
    ...
    const fieldEntries = entity.fields.map((field) => ({ ...field, parser: storageParser(field), validator: fields[field.name]?.validator }));
    ...
    function boundary(kind: Boundary, stage: "input" | "normalized" = "input"): StandardSchemaV1<StorageRow> {
      ...
      return { "~standard": { version: 1, vendor: "loom", async validate(value) {
        ...
        if (kind !== "public") for (const key of Object.keys(input)) {
          if (!allowed.has(key)) issues.push({ message: "Field is not allowed", path: [key] });
        }
        for (const field of selected) {
          const present = Object.hasOwn(input, field.name);
          const required = kind === "storage" || kind === "public" || (kind !== "patch" && field.notNull && field.defaultValue === undefined);
          ...
          if (stage === "input" && field.validator && kind !== "storage" && kind !== "public" && normalized !== null) {
            const checked = await field.validator["~standard"].validate(normalized);
            if (checked.issues) {
              if (present || required) issues.push(...checked.issues.map((issue) => ({ message: issue.message, path: [field.name, ...(issue.path ?? [])] })));
              continue;
            }
          ...
          if (!stored.success) { issues.push({ message: "Value does not fit storage", path: [field.name] }); continue; }
    ...
    validators[entity.name] = Object.freeze({ storage: boundary("storage"), insert: boundary("insert"), patch: boundary("patch"), command: boundary("command"), public: boundary("public") });
  }
