...
export function snapshotHash(snapshot: MigrationSnapshot): string {
  return createHash("sha256").update(JSON.stringify({ version: snapshot.version, dialect: snapshot.dialect,
    ddl: v.parse(snapshotValidator, snapshot).ddl.map((entity) => JSON.stringify(entity)).sort() })).digest("hex");
}

export async function createSnapshot(schema: SchemaDefinition, previous?: MigrationSnapshot): Promise<MigrationSnapshot> {
  const namespace = schema.metadata.namespace;
  const imports = namespace === "public" ? { ...schema.tables } : { ...schema.tables, __loomNamespace: pgSchema(namespace) };
...
export async function migrationStatements(before: MigrationSnapshot, after: MigrationSnapshot, renames: readonly RenameHint[] = []): Promise<readonly string[]> {
