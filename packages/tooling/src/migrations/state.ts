...
import { frameworkMigrationHash } from "./bootstrap.js";

export interface HistoryScope { readonly namespace: string; readonly metadataNamespace: string }
export interface AppliedMigration { ordinal: number; name: string; hash: string; before_hash: string; after_hash: string; catalog_hash: string }
export type HistoryIssue = "HISTORY_DIVERGED" | "LIVE_DRIFT" | "UNTRACKED_NAMESPACE" | "ORM_HISTORY_DIVERGED" | "FRAMEWORK_HISTORY_DIVERGED";
...
export async function inspectHistory(client: pg.Client, scope: HistoryScope, artifacts: readonly MigrationArtifact[]): Promise<HistoryState> {
  ...
  const history = initialized ? (await client.query<AppliedMigration>(`SELECT ordinal, name, hash, before_hash, after_hash, catalog_hash FROM ${metadata}.migration_history WHERE namespace = $1 ORDER BY ordinal`, [scope.namespace])).rows : [];
  const issues: HistoryIssue[] = [];
  if (await relationExists(client, `${metadata}.framework_migrations`)) {
    const versions = await client.query<{ version: number; hash: string }>(`SELECT version, hash FROM ${metadata}.framework_migrations ORDER BY version`);
    if (!initialized || versions.rows.length !== 1 || versions.rows[0]?.version !== 1 || versions.rows[0]?.hash !== frameworkMigrationHash(scope.metadataNamespace)) issues.push("FRAMEWORK_HISTORY_DIVERGED");
  } else {
  ...
  if (history.some((applied, index) => {
    const artifact = artifacts[index];
    return !artifact || applied.ordinal !== index + 1 || artifact.name !== applied.name || artifact.plan.hash !== applied.hash
      || artifact.plan.before !== applied.before_hash || artifact.plan.after !== applied.after_hash;
  ...
  if (await relationExists(client, drizzleName)) {
    ...
    if (recorded.rows.length !== history.length || recorded.rows.some((row, index) => row.name !== history[index]?.name || row.hash !== history[index]?.hash)) issues.push("ORM_HISTORY_DIVERGED");
