...
export function quoteIdentifier(name: string): string { return `"${v.parse(databaseIdentifier, name)}"`; }

/** Always owns a dedicated session: session locks must never return to a pool. */
export async function withMigrationConnection<T>(connectionString: string, operation: (client: pg.Client) => Promise<T>): Promise<T> {
  const url = new URL(connectionString);
  if (!["postgres:", "postgresql:"].includes(url.protocol)) throw new Error("Expected a PostgreSQL migration URL");
  const client = new pg.Client({ connectionString, connectionTimeoutMillis: 5000, application_name: "loom-migrations" });
  ...
  try {
    ...
    if (Math.floor(Number(version.rows[0]?.server_version_num) / 10000) !== 18) throw new Error("Loom migrations require PostgreSQL 18");
