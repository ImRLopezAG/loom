  ...
  const result = await client.query<{ definition: string }>(
    ...
    WITH scope AS (SELECT oid FROM pg_namespace WHERE nspname = $1), definitions AS (
      ...
      SELECT jsonb_build_array('relation', c.relname, c.relkind, pg_get_userbyid(c.relowner),
        ...
        FROM pg_class c WHERE c.relnamespace IN (SELECT oid FROM scope)
      UNION ALL
      SELECT jsonb_build_array('column', c.relname, a.attname, a.attnum, format_type(a.atttypid, a.atttypmod),
        ...
        WHERE c.relnamespace IN (SELECT oid FROM scope) AND a.attnum > 0 AND NOT a.attisdropped
      UNION ALL
      ...
      SELECT jsonb_build_array('index', c.relname, i.indisvalid, i.indisready, i.indisclustered, pg_get_indexdef(i.indexrelid))
        FROM pg_index i JOIN pg_class c ON c.oid = i.indexrelid WHERE c.relnamespace IN (SELECT oid FROM scope)
      UNION ALL
