...
import type { DiscoveredFunction } from "./discovery.js";
...
export interface ManifestFunction {
  ...
  readonly validation: { readonly args: { readonly vendor: string; readonly version: 1 }; readonly returns: { readonly vendor: string; readonly version: 1 } };
}
...
function moduleSpecifier(directory: string, filename: string): string {
  const path = relative(directory, filename).replaceAll("\\", "/").replace(/\.(?:[cm]?[jt]s)$/, ".js");
...
function references(project: LoadedProject, directory: string, visibility: "public" | "internal") {
  ...
  const imports = functions.map((entry, index) => `import type * as f${index} from ${JSON.stringify(moduleSpecifier(directory, join(project.backend, "functions", entry.modulePath)))};`);
  const contracts = functions.map((entry, index) => `  readonly ${JSON.stringify(entry.name)}: FunctionReference<${JSON.stringify(entry.definition.kind)}, ${JSON.stringify(visibility)}, StandardSchemaV1.InferInput<typeof f${index}.${entry.exportName}.args>, StandardSchemaV1.InferOutput<typeof f${index}.${entry.exportName}.returns>>;`);
  const values = Object.fromEntries(functions.map((entry) => [entry.name, { name: entry.name, kind: entry.definition.kind, visibility, version: project.version }]));
  return {
    declarations: [
      'import type { FunctionReference, StandardSchemaV1 } from "@loom/core/client";',
      ...imports, `export interface References {\n${contracts.join("\n")}\n}`,
...
function registry(project: LoadedProject, directory: string): string {
  const imports = project.functions.map((entry, index) => `import { ${entry.exportName} as f${index} } from ${JSON.stringify(moduleSpecifier(directory, join(project.backend, "functions", entry.modulePath)))};`);
  return [...imports, `export const registry = Object.freeze({\n${project.functions.map((entry, index) => `  ${JSON.stringify(entry.name)}: f${index},`).join("\n")}\n});`, ""].join("\n");
}

function manifestFunction(entry: DiscoveredFunction, version: string): ManifestFunction {
  return { name: entry.name, kind: entry.definition.kind, visibility: entry.definition.visibility,
    contractVersion: version, validation: {
      args: { vendor: entry.definition.args["~standard"].vendor, version: entry.definition.args["~standard"].version },
      returns: { vendor: entry.definition.returns["~standard"].vendor, version: entry.definition.returns["~standard"].version },
    } };
}

export async function generateProject(root: string): Promise<FunctionManifest> {
  const project = await loadProject(root);
  const generationRoot = await resolveProjectPath(project.root, relative(project.root, join(project.backend, "_generated")));
  const directory = join(generationRoot, project.version);
  const manifest: FunctionManifest = { format: 1, project: project.config.project, version: project.version, schemaFingerprint: project.schema.fingerprint,
  ...
  try { await mkdir(generationRoot); }
  catch (cause) {
  ...
  try { await writeFile(join(generationRoot, ".loom-generated"), "loom-generated-v1\n", { flag: "wx" }); }
  ...
  await mkdir(staging);
  try {
    await Promise.all(Object.entries(artifacts).map(([name, content]) => writeFile(join(staging, name), content, { flag: "wx" })));
    try { await rename(staging, directory); }
    catch (cause) {
      if (!(cause instanceof Error) || !("code" in cause) || !["EEXIST", "ENOTEMPTY"].includes(String(cause.code))) throw cause;
      for (const [name, content] of Object.entries(artifacts)) {
        if (await readFile(join(directory, name), "utf8") !== content) throw new Error("Existing generated artifacts are inconsistent with their version");
      }
    }
  ...
  try {
    ...
    if (!/^([a-f0-9]{64})$/.test(relative(generationRoot, target))) throw new Error("Refusing to replace an unmanaged _generated link");
  } catch (cause) {
