...
export interface FunctionModule { readonly path: string; readonly exports: v.InferOutput<typeof moduleNamespace> }
export interface DiscoveredFunction { readonly name: string; readonly modulePath: string; readonly exportName: string; readonly definition: FunctionMetadata }
...
export function discoverFunctions(modules: readonly FunctionModule[]): readonly DiscoveredFunction[] {
  ...
  for (const module of modules) {
    ...
    if (!/^[a-zA-Z0-9][a-zA-Z0-9_/-]*$/.test(modulePath)) throw new Error(`Invalid function module path: ${module.path}`);
    for (const [exportName, definition] of Object.entries(module.exports)) {
      if (!isRegisteredFunction(definition)) continue;
      if (!/^[a-zA-Z][a-zA-Z0-9_]*$/.test(exportName)) throw new Error(`Invalid function export: ${module.path}:${exportName}`);
