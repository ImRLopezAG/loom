...
Database acceptance, SDK target discovery and a service/worker Functions release have passed locally against a disposable Neon branch using a temporary project-scoped API key. The branch and key were removed and their absence verified afterward. The Functions check verifies release health, activation and anonymous-request rejection; authorized application operations, storage events and browser subscriptions remain outstanding. Local Functions testing requires `LOOM_CLOUD_FUNCTIONS=1` in addition to the project, disposable branch and API key variables. GitHub-hosted workflow execution is also unverified until a remote repository and its environment are configured.
